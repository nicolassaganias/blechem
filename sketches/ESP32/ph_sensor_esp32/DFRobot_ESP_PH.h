...
public:
    DFRobot_ESP_PH();
    ~DFRobot_ESP_PH();
    int calibration(float voltage, float temperature, char *cmd); //calibration by Serial CMD
...
private:
    ...
    int phCalibration(byte mode); // calibration process, wirte key parameters to EEPROM
